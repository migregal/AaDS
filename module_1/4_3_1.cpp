...
public:
    ...
    void add(const T &);
    void replace_head_with(const T &);
...
inline bool is_after(Heap<int> &stops, stop &b)
{
    return stops.size() > 0 && stops.top() < b.first;
}
...
size_t dead_ends_count(Heap<T> &heap, comp comparator)
{
    ...
    while (heap.size() > 0)
    {
        ...
        if (comparator(dead_ends, temp))
            dead_ends.replace_head_with(temp.second);
        else
...
void Heap<T>::SiftDown(size_t idx)
{
    ...
    if (left < dec.size() && dec[left] < dec[idx])
        largest = left;
    if (right < dec.size() && dec[right] < dec[largest])
        largest = right;
...
template <class T>
void Heap<T>::replace_head_with(const T &data)
{
    assert(dec.size() > 0);
